...
#define MINIMUM_SCROLL_TO_COMPLETE 100.0f // ADDED FOR FREEMINDERS
...
{
    if (_cellState == kCellStateCenter) // ADDED FOR FREEMINDERS && self.cellScrollView.contentOffset.x == [self leftUtilityButtonsWidth]
        return;
    // Scroll back to center
    
    // Force the scroll back to run on the main thread because of weird scroll view bugs
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.cellScrollView setContentOffset:CGPointMake([self leftUtilityButtonsWidth], 0) animated:animated]; // EDIT FOR FREEMINDERS
    });
    _cellState = kCellStateCenter;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    else
    {
        ...
        if ([self leftUtilityButtonsWidth] > 0)
        {
            ...
            // START ADDED FOR FREEMINDERS
            if (scrollViewWidth < - MINIMUM_SCROLL_TO_COMPLETE) {
                UIButton *button = [self.leftUtilityButtons firstObject];
                [button setSelected:YES];
            } else {
                UIButton *button = [self.leftUtilityButtons firstObject];
                [button setSelected:NO];
            }
            // END ADDED FOR FREEMINDERS
        }
        else
        {
